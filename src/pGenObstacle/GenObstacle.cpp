...
bool GenObstacle::Iterate()
{
  ...
  vector<string> iteration_obstacles = m_new_obstacle;

  while (!iteration_obstacles.empty())
  {
    // Get relevant values from message
    string current_obstacle = iteration_obstacles.back();
    iteration_obstacles.pop_back();
